...
gc.map_draw_uv(
    ...
    title="Surface current in Gulf of Tonkin",
    lon_data=lon_t,
    lat_data=lat_t,
    data_u= U1, data_v= V1,
    mask_ocean = mask_t,
    ...
    quiver_max_n=30,   # ~max arrows per axis (auto step so arrows <= quiver_max_n x quiver_max_n)
    quiver_scale=4  # higher is shorter arrow. lower is longer arrow
)

import xarray as xr
import numpy as np

# Tạo dataset, không cần khai báo tọa độ
ds = xr.Dataset(
    {
        "U1": (("y", "x"), U1),
        "V1": (("y", "x"), V1),
