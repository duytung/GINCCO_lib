...
def draw_map_plot(varname, var, lon, lat, options, state=None):
    ...
    lon_min = safe_float(options.get("lon_min"))
    lon_max = safe_float(options.get("lon_max"))
    lat_min = safe_float(options.get("lat_min"))
    lat_max = safe_float(options.get("lat_max"))

    lon_min = lon_min if lon_min is not None else float(np.nanmin(lon))
    lon_max = lon_max if lon_max is not None else float(np.nanmax(lon))
    lat_min = lat_min if lat_min is not None else float(np.nanmin(lat))
    lat_max = lat_max if lat_max is not None else float(np.nanmax(lat))

    dpi = int(options.get("dpi", 100))
    resolution = options.get("resolution", "i")

    # có thể thêm 2 option này nếu thích trunc cmap
    cmap_min = safe_float(options.get("cmap_min")) or 0.0
    cmap_max = safe_float(options.get("cmap_max")) or 1.0
